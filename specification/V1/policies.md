...
## Exclusion
An exclusion policy specifies that the AV shall not enter the escort and shall not move if it is already inside the escort. This includes preventing motion from all controlled motions - e.g. Raise Tray or Lower Tray.
...
## Speed Limit
A speed limit policy specifies that the AV shall reduce its speed to the specified limit before entering and while inside the escort.
...
> [!IMPORTANT]
> When speed limits are specified in the map or multiple overlapping speed escorts exist, the lowest speed limit applies.
